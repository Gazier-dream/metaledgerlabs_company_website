...
const SYSTEM_PROMPT = `You are Ledger, the AI assistant for MetaLedgerLabs — a Dubai-headquartered technology company that builds enterprise-grade AI and blockchain infrastructure.

Your role is to help website visitors understand MetaLedgerLabs' services, guide them to the right product or team, and encourage them to schedule a discovery call or get in contact.

About MetaLedgerLabs:
- Founded 2024, Dubai HQ, offices in London and Dubai
...
Rules: Be concise and institutional. End every reply with a next step (/schedule or /contact). Max 3-5 sentences. Never invent names or case studies. Stay on MetaLedgerLabs topics only.`;
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        background: isUser ? "var(--green)" : "var(--bg3)",
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "16px 14px 8px", scrollbarWidth: "thin", scrollbarColor: "var(--border) transparent" }}>
              ...
              {loading && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: "0.65rem" }}>
                  <div style={{ width: 28, height: 28, borderRadius: 8, flexShrink: 0, background: "var(--green-bg)", border: "1px solid var(--green-bd)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Sparkles size={13} style={{ color: "var(--green)" }} />
                  </div>
            ...
            <div style={{ padding: "10px 12px 14px", borderTop: "1px solid var(--border)", flexShrink: 0, background: "var(--bg)" }}>
              ...
              >
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.9 }}
                  style={{ width: 34, height: 34, borderRadius: 9, border: "none", background: input.trim() && !loading ? "var(--green)" : "var(--bg3)", cursor: input.trim() && !loading ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "background .15s" }}
                >
              ...
              <div style={{ fontFamily: "var(--font-body)", fontSize: "0.65rem", color: "var(--tx3)", textAlign: "center", marginTop: 7 }}>
                Powered by Groq · MetaLedgerLabs AI
              </div>
            </div>
          </motion.div>
        )}
