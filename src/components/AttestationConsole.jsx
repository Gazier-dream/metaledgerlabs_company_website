...
function ZKSeal({ size = 80 }) {
  return (
    <div style={{
      ...
      background: "conic-gradient(from 0deg, var(--green), var(--gold2), #0d9b84, var(--green))",
      boxShadow: "0 0 0 2px var(--bg2), 0 0 0 4px var(--green), 0 6px 20px rgba(8,153,129,0.4)",
      display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
      color: "var(--bg)", textAlign: "center", cursor: "default",
      transition: "transform 0.3s, box-shadow 0.3s",
      userSelect: "none",
    }}
      onMouseEnter={e => { e.currentTarget.style.transform = "scale(1.07) rotate(6deg)"; e.currentTarget.style.boxShadow = "0 0 0 2px var(--bg2), 0 0 0 4px var(--green), 0 12px 28px rgba(8,153,129,0.6)"; }}
      onMouseLeave={e => { e.currentTarget.style.transform = "scale(1) rotate(0deg)"; e.currentTarget.style.boxShadow = "0 0 0 2px var(--bg2), 0 0 0 4px var(--green), 0 6px 20px rgba(8,153,129,0.4)"; }}>
...
export default function AttestationConsole() {
  ...
  const panelHeader = (title, sub) => (
    <div style={{
      ...
      <div>
        <div style={{ fontFamily: "var(--font-body)", fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: "var(--green)", marginBottom: 2 }}>{title}</div>
            ...
            {logs.map((log, i) => (
              <motion.div
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 3 }}>
                  <code style={{ fontSize: "0.65rem", color: "var(--green)", fontFamily: "monospace" }}>{log.id}</code>
                  {log.status === "ATTESTED"
            ...
            {PROOF_STAGES.map((stage, i) => {
              ...
              return (
                <React.Fragment key={stage.key}>
                  {i > 0 && <div className="trail-arrow" style={{ borderLeftColor: done ? "var(--green-bd)" : "var(--border)" }} />}
        ...
        <div style={{ padding: "1rem", flex: 1 }}>
          ...
          <div>
            {[
              ...
              { label: "Inference Latency", value: <span style={{color:"var(--green)",fontWeight:600}}>{selectedJob.latency}</span> },
            ].map(f => (
          ...
          <div style={{
            ...
            <FileCheck size={13} style={{ color: "var(--green)", flexShrink: 0 }} />
          ...
          {NODES.map((node, i) => (
            <div key={node.id} style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
                ...
                <div style={{ fontFamily: "var(--font-body)", fontSize: "0.78rem", fontWeight: 700, color: "var(--green)" }}>{node.score}</div>
              </div>
