      ...
      <section style={{background:"var(--bg2)",borderBottom:"1px solid var(--border)",padding:"5rem 2rem 4rem",position:"relative",overflow:"hidden"}}>
        ...
        <div className="wrap" style={{position:"relative",zIndex:1}}>
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.7}}>
            ...
            <h1 className="t-h1" style={{marginBottom:"1.25rem",maxWidth:660}}>Let's Build Something <em style={{fontStyle:"italic",color:"var(--green)"}}>integrated</em> Together</h1>
            <p className="t-body-lg" style={{maxWidth:560}}>Whether you're exploring a new Web3 gaming economy, deploying Web3 gaming economy at scale, or navigating a complex digital transformation — share your challenge and we will respond with substance.</p>
          </motion.div>
        </div>
      </section>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1.4fr",gap:"5rem",alignItems:"start"}} className="ct-g">
          ...
          <motion.div initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} transition={{duration:.6}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"1.75rem",marginBottom:"3rem"}}>
              {[{icon:MapPin,l:"Headquarters",v:"Dubai"},{icon:Mail,l:"General Enquiries",v:"support@metaledgerlabs.com"},{icon:Clock,l:"Response Time",v:"Within one business day"}].map(({icon:Icon,l,v})=>(
                <div key={l} style={{display:"flex",alignItems:"flex-start",gap:"1rem"}}>
                  <div style={{width:42,height:42,borderRadius:11,background:"var(--green-bg)",display:"flex",alignItems:"center",justifyContent:"center",color:"var(--green)",flexShrink:0}}><Icon size={18}/></div>
            ...
            <div style={{background:"var(--bg3)",border:"1px solid var(--border)",borderRadius:16,padding:"2rem"}}>
              ...
              <p className="t-small" style={{marginBottom:"1.25rem"}}>Research updates, product announcements, and thought leadership on AI infrastructure and Web3 gaming economy design.</p>
              <div style={{display:"flex",gap:"0.5rem"}}>
                {[{href:"https://www.linkedin.com/company/metaledgerlabs",I:Linkedin,l:"LinkedIn"},{href:"https://github.com/orgs/MetaLedgerLabs/",I:Github,l:"GitHub"}].map(({href,I,l})=>(
          ...
          <motion.div initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} transition={{duration:.6,delay:.1}}>
            <div style={{background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:22,padding:"2.75rem",boxShadow:"var(--shadow)"}}>
              ...
              <form onSubmit={sub}>
                ...
                <button type="submit" disabled={!token||loading} className="btn btn-jade" style={{width:"100%",justifyContent:"center",marginTop:"1.25rem",padding:".9rem",fontSize:".9rem",opacity:(!token||loading)?.4:1,cursor:(!token||loading)?"not-allowed":"pointer"}}>
