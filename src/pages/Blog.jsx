...
const tagColors = ["var(--cyan)", "var(--amber)", "var(--cyan)", "var(--amber)"];
    ...
    <div style={{ background: "var(--bg)", color: "var(--tx)", minHeight: "100vh" }}>
      ...
      <section style={{ background: "var(--bg2)", borderBottom: "1px solid var(--border)", padding: "6rem 2rem 4.5rem", position: "relative", overflow: "hidden" }}>
        ...
        <div className="wrap" style={{ position: "relative", zIndex: 1 }}>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <div className="badge" style={{ marginBottom: "1.5rem", color: "var(--green)" }}>
              <span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--green)", flexShrink: 0 }} />
              Insights &amp; Research
            </div>
            <h1 className="t-h1" style={{ marginBottom: "1rem", maxWidth: 660 }}>
              Perspectives on Web3 Gaming AI, Blockchain Blockchain &amp;{" "}
              <em style={{ fontStyle: "italic", color: "var(--green)" }}>Onchain Economy</em>
            </h1>
            <p className="t-body-lg" style={{ maxWidth: 560, color: "var(--tx2)" }}>
              Original analysis from the MetaLedgerLabs team — on the architecture, regulation, and commercial reality of enterprise-grade AI and blockchain infrastructure.
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }}>
          <Link to={`/blog/${featured.slug}`} style={{ textDecoration: "none", display: "block" }}>
            <div
              ...
              className="feat-c"
              onMouseEnter={e => { e.currentTarget.style.borderColor = "var(--green)"; e.currentTarget.style.boxShadow = "var(--shadow2)"; }}
            ...
            >
              ...
              <div style={{ padding: "3rem 2.75rem", display: "flex", flexDirection: "column", justifyContent: "center", gap: "1rem" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14, paddingTop: "0.75rem", borderTop: "1px solid var(--border)" }}>
                  <div style={{ width: 36, height: 36, borderRadius: 8, background: "var(--green-bg)", border: "1px solid var(--green-bd)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  ...
                  <div className="btn btn-jade" style={{ marginLeft: "auto", whiteSpace: "nowrap" }}>Read Article <ArrowUpRight size={13} /></div>
                </div>
              </div>
            </div>
          </Link>
      ...
      <div className="wrap" style={{ paddingBottom: "1.75rem" }}>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <button
            ...
            style={{ padding: "5px 14px", borderRadius: 7, border: `1px solid ${!activeTag ? "var(--green)" : "var(--border)"}`, background: !activeTag ? "var(--green-bg)" : "transparent", fontFamily: "var(--font-body)", fontSize: ".74rem", fontWeight: 600, color: !activeTag ? "var(--green)" : "var(--tx2)", cursor: "pointer", transition: "all .15s" }}
          >
            All
          </button>
          {allTags.map((t, i) => (
      ...
      <div className="wrap" style={{ paddingBottom: "6rem" }}>
            ...
            >
              ...
              <div style={{ padding: "1.5rem", flex: 1, display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, paddingTop: "1rem", marginTop: "auto", borderTop: "1px solid var(--border)" }}>
                  <div style={{ width: 28, height: 28, borderRadius: 6, background: "var(--green-bg)", border: "1px solid var(--green-bd)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  ...
                  <Link to={`/blog/${p.slug}`} style={{ display: "inline-flex", alignItems: "center", gap: 4, fontFamily: "var(--font-body)", fontSize: ".76rem", fontWeight: 600, color: "var(--green)", textDecoration: "none", transition: "opacity .15s", flexShrink: 0 }}
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: "center", padding: "4rem 0", color: "var(--tx3)" }}>
            ...
            <button onClick={() => setActiveTag(null)} style={{ marginTop: "1rem", padding: "8px 20px", borderRadius: 8, border: "1px solid var(--border)", background: "transparent", color: "var(--green)", fontFamily: "var(--font-body)", fontSize: ".84rem", fontWeight: 600, cursor: "pointer" }}>
              Clear filter
            </button>
          </div>
        )}
      ...
      <section style={{ background: "var(--bg2)", borderTop: "1px solid var(--border)", padding: "5rem 2rem", textAlign: "center" }}>
        <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <Link to="/schedule" className="btn btn-jade">
