...
export default function BlogDetail(){
  ...
  if(!post) return(
    <div style={{maxWidth:800,margin:"0 auto",padding:"5rem 2rem",color:"var(--tx)"}}>
      ...
      <Link to="/blog" style={{display:"inline-flex",alignItems:"center",gap:6,color:"var(--cyan)",textDecoration:"none",fontWeight:600,marginTop:"1rem"}}><ArrowLeft size={15}/>Back to Insights</Link>
    </div>
  );
  return(
    <div style={{background:"var(--bg)",color:"var(--tx)"}}>
      ...
      <div style={{maxWidth:760,margin:"0 auto",padding:"3rem 2rem 6rem"}}>
        <Link to="/blog" style={{display:"inline-flex",alignItems:"center",gap:6,fontFamily:"var(--font-body)",fontSize:".72rem",fontWeight:700,letterSpacing:".18em",textTransform:"uppercase",color:"var(--cyan)",textDecoration:"none",marginBottom:"2.5rem"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,paddingBottom:"2rem",borderBottom:"1px solid var(--border)",marginBottom:"2.5rem"}}>
          <div style={{width:42,height:42,borderRadius:10,background:"var(--cyan-bg)",border:"1px solid var(--cyan-bd)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
        ...
        {post.externalLinks?.length>0&&(
          <div style={{marginTop:"2.5rem",paddingTop:"2rem",borderTop:"1px solid var(--border)"}}>
            ...
            {post.externalLinks.map((l,i)=>(
              <a key={i} href={l.url} target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",gap:6,fontFamily:"var(--font-body)",fontSize:".86rem",color:"var(--cyan)",textDecoration:"none",fontWeight:500,marginBottom:".5rem"}}>
              ...
              {related.map(p=>(
                <Link key={p.id} to={`/blog/${p.slug}`} className="card-hover" style={{textDecoration:"none",overflow:"hidden"}}>
                  ...
                  <div style={{padding:"1.25rem"}}>
                    ...
                    <div className="t-small" style={{display:"flex",alignItems:"center",gap:5}}><BookOpen size={11} style={{color:"var(--cyan)"}}/>{p.author} · {new Date(p.date).toLocaleDateString("en-US",{month:"short",year:"numeric"})}</div>
                  </div>
                </Link>
              ))}
            </div>
